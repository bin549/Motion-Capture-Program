...
//avatars = GameObject.FindObjectsOfType<Avatar>(true);
}

    private void Start()
    {
      if (appSettings.isMaleModel != 1)
{
avatar= avatars[0];
}else{
  avatar= avatars[1];
...
appSettings.SetAvatar(avatar);
appSettings.SetAvatarAnimator(avatar.GetAnimator());
